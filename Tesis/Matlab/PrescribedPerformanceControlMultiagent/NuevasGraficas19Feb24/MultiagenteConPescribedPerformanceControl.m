...
% plotea la variable gammaV
for i = 1:l
    figure(2)
    plot(t(1:end-1),gammaV(i,:),"Linewidth",2) %% 9, 20
    hold on
    grid on
    xlabel('Time [Sec]', 'FontSize', 18)
    ylabel('Variable $\gamma_{V_{ij}}$', 'Interpreter', 'latex', 'FontSize', 18)
end

% plotea la variable zeta
for i = 1:l
    figure(3)
    plot(t(1:end-1),zeta(i,:),"Linewidth",2) %% 9, 20
    hold on
    grid on
    xlabel('Time [Sec]', 'FontSize', 18)
    ylabel('Variable $\zeta_{ij}$', 'Interpreter', 'latex', 'FontSize', 18)
end