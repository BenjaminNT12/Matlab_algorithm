...
for k = 1:2
    %%%%%%%%%%%%% calculo de ep
    for i = 1: length(E)
        qtij = q(E(i,1)*3-2:E(i,1)*3) - q(E(i,2)*3-2:E(i,2)*3);
        vij = v(E(i,1)*3-2:E(i,1)*3) - q(E(i,2)*3-2:E(i,2)*3);
        qt(i*3-2:i*3) = qtij;
        eij = norm(qt(i*3-2:i*3)) - d(i);
        etaij = eij*(norm(qt(i*3-2:i*3)) + d(i));
        ep = (qtij'*vij)/(eij+d(i))
        etapij = 2*qtij'*vij
        varphi = (chi(k)*etapij-etaij*chip(k))/chi(k)^2
    end
%     %%%%%%%%%%%% calculo de etap
%     for i = 1: length(E)
%         qtij = q(E(i,1)*3-2:E(i,1)*3) - q(E(i,2)*3-2:E(i,2)*3);
...
%         ep = qt(i,:)'.*v(k,i*3-2:i*3)/(e(k,i)+d(i))
%         etap = 2*(e(k,i)+d(i))*ep
% 
%         varphi(k,i) = e(k,i)/chi(k);
%         z1(k,i) = (1/2)*log((varphi(k,i)+deltaInferiorInf)./(deltaSuperiorInf-varphi(k,i)));
%         z1p(k,i) = (1/(2*chi(k))).*(1./(varphi(k,i)+deltaInferiorInf) - 1./(varphi(k,i)-deltaSuperiorInf)).*(etap/chi(k)-(eta*chip(k))/chi(k)^2)    
% 
%     end
%     
% %     size_qt = size(qt)
